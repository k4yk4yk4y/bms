...
- **promo_manager**: чтение бонусов/проектов/permanent/retention, доступ к своему профилю.
- **shift_leader**: чтение бонусов/проектов/permanent/маркетинга/retention, доступ к профилю.
- **marketing_manager**: управление маркетингом (только свои заявки), доступ к профилю.
- **retention_manager**: управление retention, чтение бонусов/проектов/permanent, доступ к профилю.
- **smm_manager**: управление SMM, чтение бонусов/проектов/permanent, доступ к профилю.
- **delivery_manager**: чтение бонусов/проектов/permanent/retention, доступ к профилю.
- **support_agent**: чтение бонусов/проектов/permanent/маркетинга/retention, доступ к профилю.

**Ключевые секции frontend-прав:**
- `bonuses` — полный доступ к бонусам (list/show/create/update/destroy по уровню).
- `projects` — доступ к проектному функционалу (`Projects` dropdown + фильтр по `project_id`).
- `permanent_bonuses` — доступ к самой странице `/bonuses`; без этого права доступ к разделу бонусов запрещён.
  В режиме без `bonuses` показываются только permanent бонусы.
...
- `Bonuses` отображается при праве `permanent_bonuses`.
- `Heatmap` отображается только при праве `bonuses` (full bonuses access).
- `Projects` отображается при сочетании прав `permanent_bonuses` + `projects`.
...
- После входа редирект по роли:
  ...
  - admin/promo_manager/shift_leader/support_agent/delivery_manager → Bonuses
...
Доступ:
- Если у роли нет права `permanent_bonuses` → полный запрет на `/bonuses` (редирект).
- Если есть `permanent_bonuses`, но нет `bonuses` → режим **permanent-only**:
  отображаются только permanent бонусы, остальные недоступны.
- Если есть и `permanent_bonuses`, и `bonuses` → полный режим списка бонусов.

Фильтры:
- **Type/Event:** `deposit | input_coupon | manual | collection | groups_update | scheduler`.
- **Status:** `active | inactive | expired`.
- **Search:** по `name` или `code`.
- **Project:** по `project_id` (только при праве `projects`).
- **DSL Tag:** по строке.
...
- В режиме permanent-only список бонусов ограничен только permanent бонусами.
...
- `bet_level` (>= 1, только целое число).
...
- Кнопка `Copy` в блоке минимальных депозитов копирует пары `ВАЛЮТА<TAB>СУММА` (plain text).
- Кнопка `DCopy` в блоке минимальных депозитов копирует mustache-шаблон по всем доступным валютам в формате `{{#equals account_currency "usd" }}50&nbsp;&#36;{{/equals}}`.
- Для каждой награды `Freespin` добавлена отдельная кнопка `DCopy` для копирования ставок по фриспинам (`currency_freespin_bet_levels`).
- Для `Cash Bonus` (fixed, не percentage) добавлена отдельная кнопка `DCopy` для копирования сумм выдаваемого бонуса.
- Для `Cash Bonus` (percentage) добавлена отдельная кнопка `DCopy` для копирования максимальных сумм бонуса (`currency_maximum_amounts`).
- В `DCopy` используется запятая как десятичный разделитель, `&nbsp;` между числом и суффиксом валюты и HTML-entity символы валют.
- В режиме permanent-only доступ к деталям non-permanent бонуса запрещён:
  выполняется редирект на список `/bonuses` с flash-уведомлением о недостатке прав.
...
### 4.7 Heatmap (`/heatmap`)
- Доступ только при full bonuses access (право `bonuses` не `none`).
...
### 6.7 Доступы Bonuses/Projects/Permanent
- Раздел `/bonuses` защищён правом `permanent_bonuses`.
- Разделение `Projects` от `Bonuses`: проектный dropdown/фильтр виден и применяется только при праве `projects`.
- Режим permanent-only (`permanent_bonuses` есть, `bonuses` = `none`) показывает только permanent бонусы.
- Прямой переход на non-permanent бонус в permanent-only режиме приводит к редиректу на `/bonuses` и flash о недостатке прав.
